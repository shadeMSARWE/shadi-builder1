module.exports = async (plan) => {
  // جلب صور حقيقية بناءً على نوع الموقع من Unsplash
  const category = plan.category || "technology";
  const heroImage = `https://images.unsplash.com/photo-1550751827-4bd374c3f58b?auto=format&fit=crop&w=1200&q=80`; // صورة افتراضية فخمة
  const featureImage = `https://source.unsplash.com/featured/800x600?${category},software`;
...
<head>
    ...
    <style>
        body { font-family: 'Tajawal', sans-serif; background-color: #030712; color: #ffffff; }
        .glass { background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.05); }
        .hero-gradient { background: radial-gradient(circle at 50% 50%, rgba(99, 102, 241, 0.15) 0%, transparent 50%); }
    </style>
</head>
<body>
    <nav class="p-6 flex justify-between items-center border-b border-white/5 glass sticky top-0 z-50">
        <div class="text-2xl font-black bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400">
            ${plan.title}
        </div>
        ...
        <button class="bg-indigo-600 hover:bg-indigo-500 px-6 py-2 rounded-full text-sm font-bold transition shadow-lg shadow-indigo-500/20">
            ابدأ الآن
        </button>
    </nav>
    ...
    <section class="relative py-20 px-6 overflow-hidden hero-gradient">
        <div class="max-w-6xl mx-auto text-center relative z-10">
            <h1 class="text-5xl md:text-7xl font-black mb-6 leading-tight">
                مستقبل الـ ${plan.title} <br> <span class="text-indigo-500">بلمسة ذكاء اصطناعي</span>
            </h1>
            <p class="text-xl text-slate-400 mb-10 max-w-2xl mx-auto">
                هذا الموقع تم توليده بالكامل بواسطة Shadi AI Builder. حلول ذكية، تصاميم عصرية، وأداء لا يضاهى.
            </p>
            <div class="flex flex-wrap justify-center gap-4">
                <button class="bg-white text-black px-8 py-4 rounded-2xl font-black hover:bg-slate-200 transition">تجرية مجانية</button>
                <button class="glass px-8 py-4 rounded-2xl font-black hover:bg-white/5 transition">شاهد الفيديو</button>
            </div>
        </div>
    ...
    <section class="py-20 px-6 bg-[#020617]">
        <div class="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
            ${plan.sections.includes("Features") ? `
    ...
    <footer class="py-12 border-t border-white/5 text-center text-slate-500 text-sm">
        <p>© 2026 ${plan.title} - تم التطوير بواسطة Shadi AI Builder</p>
    </footer>
</body>
</html>
`;
